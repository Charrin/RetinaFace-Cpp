...
Java_com_mtcnn_1as_Face_FaceDetectionModelInit(JNIEnv *env, jobject instance,
    ...
    mLiveness = new Liveness(tFaceModelDir);
...
Java_com_mtcnn_1as_Face_LiveNess(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                      jint imageWidth, jint imageHeight, jint imageChannel) {
    //  LOGD("JNI开始检测人脸");
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回空");
    ...
    ncnn::Mat ncnn_img;
    if(imageChannel==3) {
        ncnn_img = ncnn::Mat::from_pixels_resize(faceImageCharDate,
                                                            ncnn::Mat::PIXEL_BGR, imageWidth, imageHeight, 32,
                                                            32);
    }else{
        ncnn_img = ncnn::Mat::from_pixels_resize(faceImageCharDate,
                                                  ncnn::Mat::PIXEL_RGBA2BGR, imageWidth, imageHeight, 32,
                                                  32);
    }

    std::vector<float> cls_scores;
    mLiveness->start(ncnn_img,cls_scores);
    int out_size = 2;
    //  LOGD("内部人脸检测完成,开始导出数据");
    float *scores = new float[out_size];
    for(int i=0;i<out_size;i++){
        scores[i] = cls_scores[i];
    }

    jfloatArray tScores = env->NewFloatArray(out_size);
    env->SetFloatArrayRegion(tScores,0,out_size,scores);
    //  LOGD("内部人脸检测完成,导出数据成功");
    delete[] scores;
    env->ReleaseByteArrayElements(imageDate_, imageDate, 0);
    return tScores;
...
JNIEXPORT jintArray JNICALL
Java_com_mtcnn_1as_Face_FaceDetect2(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                 jint imageWidth, jint imageHeight, jint imageChannel) {
    //  LOGD("JNI开始检测人脸");
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回空");
