...
int AnchorGenerator::FilterAnchor(const Cube* cls, const Cube* reg, const Cube* pts, std::vector<Anchor>& result) {
    ...
    for (int i = 0; i < w; ++i) {
        for (int j = 0; j < h; ++j) {
            ...
            for (int a = 0; a < anchor_num; ++a) {
                if (cls->channel(0,anchor_num + a)[id] >= cls_threshold) {
                    printf("cls %f\n", cls->channel(0,anchor_num + a)[id]);
                    ...
                    CRect2f delta(reg->channel(0, a*4+0)[id],
                            reg->channel(0, a*4+1)[id],
                            reg->channel(0, a*4+2)[id],
                            reg->channel(0, a*4+3)[id]);

                    Anchor res;
                    res.anchor = cv::Rect2f(box[0], box[1], box[2], box[3]);
                    bbox_pred(box, delta, res.finalbox);
                    printf("bbox pred\n");
                    res.score = cls->channel(0,anchor_num + a)[id];
